...
int* wet::duplicateArray_int(int *oldArr)
{	
	int kdup, xkdup, ykdup, zkdup;
	int *newArr;
	
	newArr = new int[processN];
	...
	if (rank < size/2){          //first half of the box, drop just copied over
		...
		for (k=k1 ; k< k2; k++) {
			...
			*/ 
			...
			//computeCoordinate(k);
			//cout << "-- Process " << rank <<" k= "<< k << " xk yk zk " << xk << " " << yk << " " << zk << " " << "newArr[k] = " << newArr[k] << "; kdup= "<< kdup << " xk yk zk " << xkdup << " " << ykdup << " " << zkdup <<" " << "oldArr[kdup] = " << oldArr[kdup] << endl;
	...
	if (rank >= size/2){
		...
		for (int kk=k1 ; kk< k2; kk++) {
			...
			//computeCoordinate(k);
			//cout << "<< Process " << rank <<" k= "<< kdup << " xk yk zk " << xkdup << " " << ykdup << " " << zkdup <<" " << "newArr[k] = " << newArr[kdup] <<"; kdup= "<< k << " xk yk zk " << xk << " " << yk << " " << zk << " " << "oldArr[kdup] = " << oldArr[k] << endl;
